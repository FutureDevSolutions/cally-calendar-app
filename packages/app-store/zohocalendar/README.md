...
2. Create a "Server-based Applications", set the Redirect URL for OAuth `<Cal.diy URL>/api/integrations/zohocalendar/callback` replacing [Cal.diy](https://cal.com) URL with the URI at which your application runs.
3. Fill in any information you want in the "Client Details" tab
4. Go to tab "Client Secret" tab.
5. Now copy the Client ID and Client Secret into your app keys in the [Cal.diy](https://cal.com) admin panel (`<Cal.diy>/settings/admin/apps`).
...
9. You're good to go. Now you can easily add your Zoho Calendar integration in the [Cal.diy](https://cal.com) settings at `/settings/my-account/calendars`.
